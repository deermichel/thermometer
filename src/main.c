...
#define DEGREE_CHAR 223 // char code for degree symbol on lcd1602
...
void update_data(int16_t temp, int16_t hum) {
...
int main() {
    ...
    while (1) {
        ...
        if (dht22_is_ready(&dht)) {
            update_data(dht22_get_temperature(&dht), dht22_get_humidity(&dht));
            display_needs_update = true;
            dht22_trigger_measurement(&dht); // retrigger
        } else if (dht22_has_error(&dht)) {
            dht22_reset(&dht); // reset error
        }
